---
title: "AttenuationBinFinder 4 buckets"
...
InterOptAB <- function(x) {
  ...
  pC <- sum(dtPoint2$MeanAtten>b & dtPoint2$MeanAtten<=c) /tot
  pD <- sum(dtPoint2$MeanAtten>c) /tot
  
  attenuation<-sample(c("<=a", ">a & <=b",">b & <=c", ">c"),points,replace=TRUE)
  rho<-rep(NA,points)
  WhichA <- which(dtPoint2$MeanAtten <= a)
  WhichAB <- which(dtPoint2$MeanAtten > a & dtPoint2$MeanAtten <= b)
  WhichBC <- which(dtPoint2$MeanAtten > b & dtPoint2$MeanAtten <= c)
  WhichC <- which(dtPoint2$MeanAtten > c)
  
  SampA <- dtPoint2[sample(WhichA, length(rho[attenuation=="<=a"]), replace = TRUE), ]
  SampB <- dtPoint2[sample(WhichAB, length(rho[attenuation==">a & <=b"]), replace = TRUE), ]
  SampC <- dtPoint2[sample(WhichBC, length(rho[attenuation==">b & <=c"]), replace = TRUE), ]
  SampD <- dtPoint2[sample(WhichC, length(rho[attenuation==">c"]), replace = TRUE), ]
  
  rho[attenuation=="<=a"]<- SampA$Distance
  rho[attenuation==">a & <=b"]<- SampB$Distance
  rho[attenuation==">b & <=c"]<-SampC$Distance
  rho[attenuation==">c"]<-SampD$Distance
  # 
  ...
  frame$Dose[frame$attenuation == ">a & <=b"] <- (!SampB$Block) * frame$Dose[frame$attenuation == ">a & <=b"]
  frame$Dose[frame$attenuation == ">b & <=c"] <- (!SampC$Block) * frame$Dose[frame$attenuation == ">b & <=c"]
  frame$Dose[frame$attenuation == ">c"] <- (!SampD$Block) * frame$Dose[frame$attenuation == ">c"]
  ...
  dtC <- frame %>% filter(attenuation == ">b & <=c")
  
  dtD <- frame %>% filter(attenuation == ">c")
  ...
  DistOut <- sqrt( (2*pA*pB)*((ExA - ExB)^2) + (2*pB*pC)*((ExB - ExC)^2) + (2*pC*pD)*((ExC - ExD)^2) + (2*pD*pA)*((ExD - ExA)^2))
  #data.frame(ExpDose = c(ExDA,ExDB,ExDC), LocProb = c(pA,pB,pC),Metric = rep(DistOut,3))
  return(data.frame(ExpDose = c(ExA,ExB,ExC,ExD), BinFreq = c(pA,pB,pC,pD),Metric = rep(DistOut,4)))
}
...
nextfun <- function(x) {
  a = 50# sample(32:80,1)
  b = sample((a+6):90,1) # These values are arbitrary, but it helps to keep them in the bulk of the (pseudo)data 31.5 < a, b < 110
  c = sample((b+6):100,1) # Gap of 10 for sensitivity purposes
  return(c(a,b,c))
}
...
df.max <- head(df.sorted,20)
df.maxtest <- df.max

for (i in 1:length(df.max$out)) {
  vec.pair <- c()
  for (j in 1:100) {
    vec.pair <- c(vec.pair, InterOptAB(c(df.max$tup.1[i],df.max$tup.2[i],df.max$tup.3[i])))
  }
